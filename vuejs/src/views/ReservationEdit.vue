<template>
  <div class="offerReservation">
    <b-container>
      ...
      <date-picker v-model="time1" :disabled-date="validDate" :disabled-time="validTime" :minute-step="15"
                   :placeholder="$t('VALID_FROM')"
                   format="HH:mm" type="datetime" v-on:change="time2=time1"></date-picker>
      <date-picker v-model="time2" :disabled-date="validDateSecond" :disabled-time="validTimeSecond" :minute-step="15"
                   :placeholder="$t('VALID_TO')" format="HH:mm" type="datetime"></date-picker>
...
export default {
  ...
  methods: {
    ...
    getReservationInfo() {
      apiReservation.getReservation(this.$route.params.id).then(value => {
        console.log(this.$route.params.id)
        this.reservation = value;
        this.time1 = this.reservation.reservationFrom
        this.time2 = this.reservation.reservationTo
        console.log(this.reservation)
        api.get('moo/offer/withDetails/' + this.reservation.offer).then(response => {
    ...
    submitReservation() {
      apiReservation.editReservation(this.reservation.id, this.reservation.version, this.time1.toISOString(), this.time2.toISOString()).then(() => {
        this.$router.back();
      })
    }
  },
